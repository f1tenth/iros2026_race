...
def get_default_config() -> dict:
    ...
    return {
        ...
        "orientation_2": {
            ...
            "date_override": "",
            "description": (
                "Roboracer Orientation 2 ( Track set up, Track overview for in-person "
                "competition, Teams Training )"
            ),
            "zoom_link": "",
            "slides_link": "",
            "video_link": "",
...
class RepositoryUpdater:
    ...
    def _update_timeline_html(self, content: str) -> str:
        ...
        if self.dates:
            ...
            o1_description = self.o1.get(
                "description", "Roboracer Orientation 1 ( Competition Rules overview )"
            )
            o1_label = f'<span style="font-weight:inherit;font-style:inherit">{o1_description}</span>'
            if o1_zoom:
                o1_title = f'<a href="{o1_zoom}">{o1_label}</a>'
            else:
                o1_title = o1_label
            ...
            o1_row = self._clean_html(f'''<tr>
							...
							<td class="tg-j1gp">{o1_title}{o1_resources_html}
							</td>
						</tr>''')
            ...
            # Build orientation title - only link if zoom_link is provided
            o2_description = self.o2.get(
                "description",
                "Roboracer Orientation 2 ( Track set up, Track overview for in-person "
                "competition, Teams Training )",
            )
            o2_label = f'<span style="font-weight:400;font-style:normal">{o2_description}</span>'
            if o2_zoom:
                o2_title = f'<a href="{o2_zoom}">{o2_label}</a>'
            else:
                o2_title = o2_label
            ...
            o2_row = self._clean_html(f'''<tr>
							...
							<td class="tg-npj4">{o2_title}{o2_resources_html}
							</td>
						</tr>''')
...
<td class="{td2}"><span style="font-weight:400;font-style:normal;text-decoration:none;color:#000;background-color:transparent">{description}</span>
    ...
    def _update_registration_html(self, content: str) -> str:
        ...
        # otherwise the sentence renders with a blank edition and an empty href.
        if self.sim.get("enabled", False):
