...
(`null` = auto-detect). The value is clamped to 1.0–4.0 and applied on launch.
...
3. **Orientations Tab** - Configure orientation meeting links:
   - Timeline Description (the row text on the Timeline page; falls back to the
     generic "Competition Rules overview" / "Track set up ..." wording if blank)
   - Zoom meeting links
...
Adjust these offsets in the Dates & Timeline tab as needed. These are only the
factory defaults used by "Reset to Defaults" — the values in force for this event
live in `event_config.json` under `dates.offsets`.
...
- `challonge_participants.py` - Exports certified teams as `Name, leader_email`
  lines for Challonge bulk import (run: `uv run python Utils/challonge_participants.py`)
- `serve.sh` - Serves the Jekyll site locally at http://localhost:4000
